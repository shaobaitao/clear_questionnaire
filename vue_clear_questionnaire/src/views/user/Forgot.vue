  ...
  <div class="forgot">
    ...
    <van-tabs v-model="active">
      <van-tab title="通过邮箱找回">
        <van-form validate-first @submit="emailForgot">
          <van-field
              v-model="email"
              name="email"
              placeholder="邮箱"
      ...
      <!--      <van-tab title="通过手机找回">-->
      <!--        <van-form @submit="phoneForgot">-->
      <!--          <div class="inputDiv">-->
      <!--            <van-field-->
      <!--                v-model="phoneNumber"-->
      <!--                name="phoneNumber"-->
  ...
  methods: {
    ...
    emailForgot(values) {
      ...
      api.emailChange({
        ...
        response.data.code === 10203 && (this.email = this.emailVCode = this.password = '')
        Toast({
          message: response.data.msg,
          position: 'bottom',
        })
